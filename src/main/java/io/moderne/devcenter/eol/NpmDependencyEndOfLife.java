...
/**
 * A DevCenter card that reports the end-of-life status of an organization's <strong>npm</strong>
 * dependencies, read from {@code package.json}. npm has no resolved-dependency LST marker, so the
 * declared version ranges (e.g. {@code ^18.2.0}) are matched by their leading concrete version.
 */
@Value
@EqualsAndHashCode(callSuper = false)
public class NpmDependencyEndOfLife extends AbstractEndOfLifeCard {

    @Option(displayName = "Card name",
            description = "The display name of the DevCenter card.",
            example = "npm dependency end of life")
    String cardName;
    ...
    @Option(displayName = "Fix recipe",
            ...
            required = false,
            example = "org.openrewrite.nodejs.UpgradeDependencyVersion")
    @Nullable
    String fixRecipe;

    String displayName = "Node.js npm dependency end of life";

    String description = "Determine the end-of-life status of an organization's npm dependencies " +
                         "declared in package.json, sourced from a Moderne EOL feed.";
    ...
    protected Collection<Detected> detect(SourceFile source) {
        return NpmManifest.dependencies(source);
    }
