...
/**
 * A DevCenter card that reports the end-of-life status of an organization's <strong>NuGet</strong>
 * dependencies, read from {@code <PackageReference>} entries in {@code *.csproj} project files.
 * NuGet has no resolved-dependency LST marker, so the declared versions are used.
 */
@Value
@EqualsAndHashCode(callSuper = false)
public class NuGetDependencyEndOfLife extends AbstractEndOfLifeCard {

    @Option(displayName = "Card name",
            description = "The display name of the DevCenter card.",
            example = "NuGet dependency end of life")
    String cardName;
    ...
    @Option(displayName = "Products",
            description = "Optional allow-list of feed product names to limit tracking to. When " +
                          "omitted, all products in the feed are tracked.",
            required = false,
    ...
    @Option(displayName = "Fix recipe",
            description = "Optional id of a recipe that remediates this card's findings, surfaced as " +
                          "a one-click fix in DevCenter. When omitted, the card is report-only.",
            required = false,
            example = "org.openrewrite.csharp.dependencies.UpgradeDependencyVersion")
    @Nullable
    String fixRecipe;

    String displayName = "NuGet dependency end of life";

    String description = "Determine the end-of-life status of an organization's NuGet dependencies " +
                         "declared in .csproj project files, sourced from a Moderne EOL feed.";
    ...
    protected Collection<Detected> detect(SourceFile source) {
        return NuGetProject.dependencies(source);
    }
